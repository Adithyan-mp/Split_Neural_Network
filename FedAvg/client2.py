...
batch_size = 250
transform = ToTensor()
dataset = TrainDataset(path_x='/fed/x_fed_2.npy',path_y='/fed/y_fed_2.npy',transform=transform)
...
#training loop
def run(num_epoch,round,curr_round, model=model, optim=optim,
        dataloader=dataloader, 
        criterion=criterion):
    recv_model(model=model,src=3)
    ...
    for epoch in range(num_epoch):
        for i, (feature,target) in enumerate(dataloader):
            ...
            print(f"round : {curr_round}/{round} epoch : {epoch}/{num_epoch} {i}/{len(dataloader)} loss : {loss}")
    
    send_model(model=model,dst=3)
...
for i in range(round):
    run(num_epoch,round=round,curr_round=i)
